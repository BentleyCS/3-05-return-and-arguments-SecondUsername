...
#and return statements. our task is to take each one and convert it to arguments and returns instead.


#modify the below function such that it asks the user for 2 numbers as input.
#Then have it return out the larger number
def larger(n1, n2):
    n1 = int(n1)
    n2 = int(n2)
    if (n1 > n2):
...
#Then based on the score return out a letter grade.
# 90+ A
# 80+ B
# 70+ C
# 60+ D
...
#if both are the case then return "Fizzbuzz" instead of the prior two
#if niether are the case return the number.
def fizzBuzz(n):
    n = int(n)
    if(n%5==0 and n%3==0):
        return( "FizzBuzz")
